...
def train_net(local_rank,nprocs, args):
    ...
    if local_rank ==0:
        ...
        memory3 = ClusterMemory(args.feat_dim, cfg.MODEL.NUM_CLASSES,local_rank,temp=args.temp,
                            momentum=args.memory_momentum, use_hard=args.use_hard)
        
        memory = memory3
        class_meter = class_meter3
        
    elif local_rank ==3:
        class_meter4 = TestMeter(
        len(class_loader.dataset.data)// (cfg.TEST.NUM_ENSEMBLE_VIEWS * cfg.TRAIN.NUM_SPATIAL_CROPS),
        cfg.TEST.NUM_ENSEMBLE_VIEWS * cfg.TRAIN.NUM_SPATIAL_CROPS,
        cfg.MODEL.NUM_CLASSES,
        len(class_loader),
        ...
        memory4 = ClusterMemory(args.feat_dim, cfg.MODEL.NUM_CLASSES,local_rank,temp=args.temp,
                            momentum=args.memory_momentum, use_hard=args.use_hard)
        memory = memory4
        class_meter = class_meter4
