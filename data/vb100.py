...
class vb100(torch.utils.data.Dataset):
    """
    ...
    def __init__(self,cfg=None,num_frames=8,train=True, labelled=True, target_transform=None, loader=default_loader, download=True,fraction=1.0):
        ...
        self.cfg = cfg
        self.root = cfg.DATA.vb100_PATH_TO_DATA_DIR
    ...
    def _construct_loader(self):
        ...
        if self.mode == "train":
            if self.labelled == True:
                label_file_path = "vb100_label_train.json"
            else:
                label_file_path = "vb100_unlabel_train.json"
        else:
            label_file_path = "vb100_label_validation.json"


        label_file = os.path.join(self.cfg.DATA.vb100_PATH_TO_DATA_DIR,label_file_path)
        ...
        if self.mode == "train":
            if self.labelled == True:
                csv_file_path = "vb100_label_train.csv"
            else:
                csv_file_path = "vb100_unlabel_train.csv"
        else:
            csv_file_path = "vb100_label_validation.csv"
        
        path_to_file = os.path.join(
            self.cfg.DATA.vb100_PATH_TO_DATA_DIR, csv_file_path
        )
        ...
        (self._path_to_videos, _) = utils.load_image_lists(
            path_to_file, self.cfg.DATA.vb100_PATH_PREFIX, return_list=True
        )
        ...
        cub_csv_file = os.path.join(
            self.cfg.DATA.Ucf101_PATH_TO_DATA_DIR, cub_file_path)
...
def get_vb100_datasets(cfg,num_frames):
    
    class_dataset = vb100(cfg=cfg,num_frames=num_frames,train=True,labelled=False,download=False,fraction=0.1)
    class_label_dataset = vb100(cfg=cfg,num_frames=num_frames,train=True,labelled=True,download=False,fraction=0.3)
    train_dataset_labelled = vb100(cfg=cfg,num_frames=num_frames,train=True,labelled=True,download=False)
    train_dataset_unlabelled = vb100(cfg=cfg,num_frames=num_frames,train=True,labelled=False,download=False)
    test_dataset = vb100(cfg=cfg,num_frames=num_frames, train=False,labelled=True, download=False)
