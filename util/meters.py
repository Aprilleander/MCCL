...
from util.cluster_and_log_utils import split_cluster_acc_v2_balanced,split_cluster_acc_v2
...
class TestMeter(object):
    ...
    def __init__(
        ...
        self.mask = torch.zeros((num_videos)).bool()
        self.topk_accs = []
        self.stats = {}
    ...
    def reset_pred(self,new_pred):
        """
        Reset the metric.
        """
        
        self.video_preds = new_pred
        

    def update_stats(self, preds, labels, mask,clip_ids):
        """
    ...
    def finalize_metrics(self, ks=(1, 5)):
        ...
        if self.multi_label:
            ...
            num_topks_correct,pred_ = metrics.topks_correct(
                self.video_preds, self.video_labels, ks
            )

            pred_ = pred_[0]
            label_ = self.video_labels
            mask_ =self.mask
            #targets = self.video_labels[:468]

            print(pred_)
            print(label_)

            total_acc, old_acc, new_acc = split_cluster_acc_v2(y_true=label_,y_pred=pred_,mask=mask_)
    ...
    def finalize_metrics_balanced(self, ks=(1, 5)):
        """
        Calculate and log the final ensembled metrics.
        ks (tuple): list of top-k values for topk_accuracies. For example,
            ks = (1, 5) correspods to top-1 and top-5 accuracy.
        ...
        if self.multi_label:
            ...
            total_acc, old_acc, new_acc = split_cluster_acc_v2_balanced(y_true=label_,y_pred=pred_,mask=mask_)
    ...
    def finalize_preds(self, ks=(1, 5)):
        """
        Calculate and log the final ensembled metrics.
        ks (tuple): list of top-k values for topk_accuracies. For example,
            ks = (1, 5) correspods to top-1 and top-5 accuracy.
