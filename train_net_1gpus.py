...
from util.cluster_memory_utils import extract_features,generate_cluster_features,extract_labeled_protos,extract_labeled_stprotos,add_gaussian_noise,feature_dropout,kmeansMLPProjector

from config import exp_root
from util.loss import DINOHead, info_nce_logits, SupConLoss, DistillLoss, ContrastiveLearningViewGenerator, get_params_groups,info_nce_logits_for,LabelSmoothingLoss
from backbone import mvit

import argparse
import random
from backbone.mvit import MViT
from backbone.MViT_defaults import get_cfg
from backbone_times.vit import vit_base_patch16_224 as vit_times
from backbone_times.vit import TimeSformer as TimeSformer
from backbone_times.Timesformer_defaults import get_timesformer_cfg
from backbone_times.Timesformer_defaults import _assert_and_infer_cfg,load_config
...
def epoch_train(label_meter,class_meter,backbone,projector,memory,train_loader,unlabelled_train_loader,class_loader,uq_index,preds_ind_list,cluster_protos_list,cluster_distances_list,cluster_radius_list, args,cfg):
    ...
    if args.load_legancy:    
        ...
        backbone, projector, b_optimizer, p_optimizer, epoch = load_simgcd_legancy(backbone, projector, b_optimizer, p_optimizer, checkpoint_dir, multi_gpu=True, strict=False)
        new_epoch = epoch
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            if args.mixup_enable:
                images, class_labels = mixup_fn(images, class_labels)
            ...
            with torch.cuda.amp.autocast(fp16_scaler is not None):
                ...
                student_proj_ = feature_dropout(student_proj)
                new_student_proj = torch.cat((student_proj,student_proj_),dim=0)
                # concate_lab = torch.cat([mask_lab,mask_lab])
                student_proj = torch.cat([f[mask_lab].unsqueeze(1) for f in new_student_proj.chunk(2)], dim=1)
                ...
                loss = 0
                base_contrast_loss =0
                loss += (1 - args.sup_weight) * cluster_loss + args.sup_weight * cls_loss

                concate_labels = class_labels.cuda(local_rank,non_blocking=True)

                # concate_labels = torch.cat((class_labels,class_labels),dim=0).cuda(local_rank,non_blocking=True)
                
                # memory = update_prototype(backbone,memory,class_meter,args,cfg)
                known_memory = student_out[mask_lab]
                known_label = concate_labels[mask_lab]
                
                memory_loss = memory(known_memory,known_label)
                base_memory_loss=memory.k*memory_loss.cuda(local_rank,non_blocking=True)

                # loss += contrastive_loss

                loss += (args.sup_weight)*base_memory_loss
        ...
        if local_rank == 0 and epoch%1==0:
            ...
            print("Kmeans result...")
            all_acc, old_acc, new_acc = test_kmeans(backbone, test_loader, epoch,"Test all acc", args,projection_head=None, Use_GPU=False)
...
def test(test_meter,backbone,test_loader, epoch, save_name, args,cfg):
...
def train_net(local_rank,nprocs, args):
    ...
    if args.weight_path is not None:
        cu.load_checkpoint(args.weight_path, backbone,data_parallel=False)
    """
    ...
    uq_index, all_preds, cluster_protos_list, preds_ind_list = extract_labeled_stprotos(backbone,class_merge_loader, merge_len,args,local_rank)
    save_results(uq_index, all_preds, cluster_protos_list, preds_ind_list, save_path=cfg.OUTPUT_DIR+args.dataset_name+"sharp_kmeans.pth")
    
    # results = load_results(cfg.OUTPUT_DIR+"results.pth", local_rank)  # 或 "cpu" 视需求而定
    ...
    memory = class_vector(backbone,projector,memory,class_loader,class_meter,args,cfg)
    uq_index, preds_ind_list,cluster_protos_list,cluster_distances_list,cluster_radius_list = None,None,None,None,None
    
    epoch_train(label_meter,class_meter,backbone,projector,memory,train_loader,test_loader_unlabelled,class_loader,uq_index, preds_ind_list,cluster_protos_list,cluster_distances_list,cluster_radius_list,args,cfg)
